...
```bash
pip install artenic-ai-sdk              # core only (schemas, types, decorators)

# Install sub-packages via extras
pip install artenic-ai-sdk[ensemble]    # + EnsembleManager & 5 strategies
pip install artenic-ai-sdk[training]    # + 8 training callbacks & serialization
pip install artenic-ai-sdk[client]      # + PlatformClient (httpx)
...
> Requires `artenic-ai-sdk[ensemble]` or `artenic-ai-sdk-ensemble`.

```python
# Via conditional re-export (when installed)
from artenic_ai_sdk import EnsembleManager, WeightedAverage

# Or direct import
from artenic_ai_sdk_ensemble import EnsembleManager, WeightedAverage
...
| `PlatformClient` | Async HTTP client for the platform gateway *(requires `artenic-ai-sdk-client`)* |
| `ModelSerializer` | Save/load model weights in multiple formats *(requires `artenic-ai-sdk-training`)* |

### Ensemble *(requires `artenic-ai-sdk-ensemble`)*
...
# Run all SDK tests (core + ensemble + training + client — 610 tests)
...
# Coverage (100% required, all 4 sub-packages)
uv run pytest packages/sdk/ \
  --cov=artenic_ai_sdk \
  --cov=artenic_ai_sdk_ensemble \
  --cov=artenic_ai_sdk_training \
  --cov=artenic_ai_sdk_client \
  --cov-fail-under=100
```
