...
from artenic_ai_platform.db.models import Base, MLModel, ModelHealthRecord
...
async def _seed_ml_model(
    factory: async_sessionmaker,  # type: ignore[type-arg]
    model_id: str,
) -> None:
    """Insert a minimal MLModel row to satisfy the FK constraint."""
...
class TestCheckAllModelsPersists:
    ...
    async def test_check_all_models_persists(
        ...
        await _seed_ml_model(db_factory, model_id)
