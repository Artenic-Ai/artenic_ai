...
from artenic_ai_platform.providers_hub.public_catalog.scaleway import ScalewayCatalogFetcher
...
class TestScalewayCatalogComputeLive:
    async def test_live_success(self) -> None:
        ...
        with patch("artenic_ai_platform.providers_hub.public_catalog.scaleway.httpx") as mock_httpx:
    ...
    async def test_live_skips_non_dict_server(self) -> None:
        ...
        with patch(
            "artenic_ai_platform.providers_hub.public_catalog.scaleway.httpx",
        ) as mock_httpx:
            mock_httpx.get.return_value = mock_resp
            result = await fetcher.fetch_compute_catalog()
        assert len(result) == 1
    ...
    async def test_live_no_httpx(self) -> None:
        fetcher = ScalewayCatalogFetcher()
        with patch("artenic_ai_platform.providers_hub.public_catalog.scaleway.httpx", None):
            result = await fetcher.fetch_compute_catalog()
        assert fetcher.supports_live_catalog() is False
        assert len(result) >= 8
